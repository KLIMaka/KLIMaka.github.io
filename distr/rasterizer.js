define(["require", "exports", './libs/mathutils', './libs_js/glmatrix'], function (require, exports, MU, GLM) {
    ...
    function triangle(color, points) {
        var maxy = Math.min(Math.max(points[1], points[3], points[5]), 480);
        var miny = Math.max(Math.min(points[1], points[3], points[5]), 0);
        var dx1 = points[0] - points[2], dx2 = points[2] - points[4], dx3 = points[4] - points[0];
        var dy1 = points[1] - points[3], dy2 = points[3] - points[5], dy3 = points[5] - points[1];
        for (var y = miny; y <= maxy; y++) {
            var s1 = y - points[1], s2 = y - points[3], s3 = y - points[5];
            var x1 = points[0] + s1 / dy1 * dx1, x2 = points[2] + s2 / dy2 * dx2, x3 = points[4] + s3 / dy3 * dx3;
            var lx = Math.max(Math.min(s1 * s2 <= 0 ? x1 : Infinity, s2 * s3 <= 0 ? x2 : Infinity, s3 * s1 <= 0 ? x3 : Infinity), 0);
            var rx = Math.min(Math.max(s1 * s2 <= 0 ? x1 : -Infinity, s2 * s3 <= 0 ? x2 : -Infinity, s3 * s1 <= 0 ? x3 : -Infinity), 639);
            var yoff = y * 640;
    ...
    function sort(p1, p2, p3) {
        var r = [p1, p2, p3];
        if (r[1][1] <= r[0][1] && r[1][1] <= r[2][1])
            swap(r, 0, 1);
        if (r[2][1] <= r[0][1] && r[2][1] <= r[1][1])
            swap(r, 0, 2);
        if (r[2][1] < r[1][1])
            swap(r, 1, 2);
        if (r[2][0] < r[1][0])
    ...
    function triangle1(color, p1, p2, p3) {
    ...
    function delata(p1, p2, p3, attrsCount) {
    ...
    function line(y, p1, p2, p3, d, attrsCount) {
    ...
    function triangleZ(color, p1, p2, p3) {
    ...
    function swFastRects(count, size) {
        while (count--) {
            var color = 0xff000000 | rand256() | (rand256() << 8) | (rand256() << 16);
    ...
    function swTriangles(count, size) {
        while (count--) {
