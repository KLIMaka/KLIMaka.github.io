define(["require", "exports", './libs/browser', './modules/ui/filereader', './modules/engines/raven', './modules/pixelprovider', './libs/imgutils', './libs/mathutils'], function(require, exports, browser, files, Raven, PP, IU, MU) {
    var pal = null;
    var res = null;

    new files.DropFileReader(document.getElementById('palLoader'), function (f) {
    ...
    new files.DropFileReader(document.getElementById('resLoader'), function (f) {
        return true;
    }, function (buff) {
        res = buff;
        if (pal != null)
    ...
    var AnimController = (function () {
        ...
        AnimController.prototype.animate = function (dt) {
            ...
            if (this.angToIdx(this.ang) != this.angToIdx(rot)) {
                var sign = this.ang > rot ? (this.ang - rot > 0.5 ? 1 : -1) : (rot - this.ang > 0.5 ? -1 : 1);
                var ang = this.ang;
                ang += sign * dt * 2;
    ...
    function render(palbuf, resbuf) {
        ...
        canvas.onclick = function (e) {
            return c.goto(e.x - canvas.offsetLeft, e.y - canvas.offsetTop);
        };
