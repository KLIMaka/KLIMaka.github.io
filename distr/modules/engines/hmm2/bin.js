define(["require", "exports", '../../../libs/imgutils', '../../../libs/mathutils', '../../pixelprovider'], function(require, exports, IU, MU, pixel) {
    var shadow = [0, 0, 0, 127];

    var BinFile = (function () {
        function BinFile(data, pal) {
            this.data = data;
            this.pal = pal;
            this.goff = data.mark();
        }
        BinFile.prototype.render = function (icnProvider) {
            var s = this.data;
        ...
        BinFile.prototype.parseParts = function (canvas, icnProvider) {
            var s = this.data;
            for (; ;) {
                var type = s.readUShort();
                if (type == 0)
        ...
        BinFile.prototype.parseBg = function (canvas, icnProvider) {
            var s = this.data;
            ...
            var pp = new pixel.RGBPalPixelProvider(bg, this.pal, info.width, info.height, 255, 0, 1, shadow);
            IU.blendToCanvas(pp, canvas, xoff + info.offsetX, yoff + info.offsetY);
        };

        BinFile.prototype.parseButton = function (canvas, icnProvider) {
            var s = this.data;
            var xoff = s.readUShort();
            var yoff = s.readUShort();
            var w = s.readUShort();
            ...
            var pp = new pixel.RGBPalPixelProvider(frame, this.pal, info.width, info.height, 255, 0, 1, shadow);
            IU.blendToCanvas(pp, canvas, xoff + info.offsetX, yoff + info.offsetY);
        };

        BinFile.prototype.parseText = function (canvas, icnProvider) {
            var s = this.data;
            var xoff = s.readUShort();
            var yoff = s.readUShort();
            var w = s.readUShort();
        ...
        BinFile.prototype.parseImg = function (canvas, icnProvider) {
            var s = this.data;
            var xoff = s.readUShort();
            var yoff = s.readUShort();
            var w = s.readUShort();
