...
define(["require", "exports", './modules/engines/hmm2/agg', './modules/engines/hmm2/icn', './modules/engines/hmm2/objn', './modules/engines/hmm2/mp2', './modules/engines/hmm2/til', './libs/getter', './modules/pixelprovider', './libs/imgutils', './libs/mathutils', './libs/browser'], function(require, exports, AGG, ICN, OBJN, MP2, TIL, getter, pixel, IU, MU, browser) {
    ...
    function renderOffset(pp, x, y, off, curr) {
    ...
    var ShadowBlendPixelProvider = (function (_super) {
        __extends(ShadowBlendPixelProvider, _super);
        function ShadowBlendPixelProvider(provider) {
            _super.call(this, provider.getWidth(), provider.getHeight());
            this.provider = provider;
        }
        ShadowBlendPixelProvider.prototype.putToDst = function (x, y, dst, dstoff) {
        ...
        return ShadowBlendPixelProvider;
    })(pixel.AbstractPixelProvider);

    var TilePixelProvider = (function (_super) {
        __extends(TilePixelProvider, _super);
        function TilePixelProvider(info, details) {
            _super.call(this, details.w, details.h);
            this.info = info;
            this.details = details;
        }
        TilePixelProvider.prototype.putToDst = function (x, y, dst, dstoff) {
            var d = this.details;
            var infos = this.info;
            var a = 0;
            dst[dstoff + 3] = a;
        ...
        return TilePixelProvider;
    })(pixel.AbstractPixelProvider);

    function addonsort(a, b) {
        var dl = a.l - b.l;
        return dl == 0 ? b.q - a.q : dl;
    }

    getter.loader.load(RES).load(MAP).finish(function () {
        ...
        var icnCache = {};
        function createTile(obj, idx, count, level, adds, tile) {
            if (obj == 0)
                return;
        ...
        for (var i = 0; i < tiles.length; i++) {
            ...
            var pp = new pixel.RGBPalPixelProvider(frame, pal, tilFile.width, tilFile.height, 255, 0, 1, shadow);
            if (tile.shape % 4 == 1)
                pp = pixel.yflip(pp);
